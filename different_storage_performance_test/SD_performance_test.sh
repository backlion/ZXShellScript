...
do
	...
	echo "%%% SD to emmc test : $i time %%%" >> different_storage_copy-compare_test.log
	date
...
do
	...
	then
		...
		echo "delete /storage/extsdcard/CPTEMP folder" >> different_storage_copy-compare_test.log
	fi
	echo "%%% SD to SD test : $i time %%%"
	echo "%%% SD to SD test : $i time %%%" >> different_storage_copy-compare_test.log
	date
	date >> different_storage_copy-compare_test.log
...
then
	...
	do
		...
		echo "%%% SD to USB test : $i time %%%" >> different_storage_copy-compare_test.log
		date
		date >> different_storage_copy-compare_test.log
		...
		then
			...
			echo "finish copy /storage/extsdcard/CPFILE to /storage/usbdisk1" >> different_storage_copy-compare_test.log
		else
			echo "error when copy /storage/extsdcard/CPFILE to /storage/usbdisk1"
			echo "error when copy /storage/extsdcard/CPFILE to /storage/usbdisk1" >> different_storage_copy-compare_test.log
		fi
		speed=`expr $(expr $size /* 1000) / $cp_time`
		let total+=speed
		echo "speed of $i time: $speed byte/s = `expr $speed / 1024` KB/s = `expr $speed / 1048576` MB/s"
